...
for index, axis in enumerate(["x", "y", "z"]):
    ...
    real.plot(label="Leader", use_index=True, ax=ax, grid=True, color="#D95319")

    smile_boy = data[f"/smile_boy/mavros/local_position/odom/pose/pose/position/{axis}"].dropna()
    smile_boy.plot(label="Follower1", use_index=True, ax=ax, grid=True, color="#EDB120")

    xiao_feng = data[f"/xiao_feng/mavros/local_position/odom/pose/pose/position/{axis}"].dropna()
    xiao_feng.plot(label="Follower2", use_index=True, ax=ax, grid=True, color="#7E2F8E")
    # Set the axis labels
...
for index, axis in enumerate(["x", "y", "z"]):
    ...
    smile_boy = data[f"/smile_boy/mavros/local_position/odom/twist/twist/linear/{axis}"].dropna()
    smile_boy.plot(label="Follower1", use_index=True, ax=ax, grid=True, color="#EDB120")

    xiao_feng = data[f"/xiao_feng/mavros/local_position/odom/twist/twist/linear/{axis}"].dropna()
    xiao_feng.plot(label="Follower2", use_index=True, ax=ax, grid=True, color="#7E2F8E")
    # Set the axis labels
    ax.set_xlabel("Time t (s)")
...
for index, axis in enumerate(["x", "y", "z"]):
    ...
    smile_boy = data[f"/smile_boy/mavros/local_position/odom/twist/twist/angular/{axis}"].dropna()
    smile_boy.plot(label="Follower1", use_index=True, ax=ax, grid=True, color="#EDB120")

    xiao_feng = data[f"/xiao_feng/mavros/local_position/odom/twist/twist/angular/{axis}"].dropna()
    xiao_feng.plot(label="Follower2", use_index=True, ax=ax, grid=True, color="#7E2F8E")
